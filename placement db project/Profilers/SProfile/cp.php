...
mysql_select_db("Revised"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysql_query("SELECT * FROM `Revised`.`slogin` WHERE `USN`='$Username'");
			if(mysql_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysql_query("UPDATE `Revised`.`slogin` SET `PASSWORD` = '$Password' WHERE `slogin`.`USN` = '$Username'"))
					{
